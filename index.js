...
function createTicket (ticketColor, ticketDesc, ticketId) {
    //either choose ticketId (existing) or shortid (new)
...
function handleRemove(ticket) {
    ticket.addEventListener('click', event => {
        ...
        //if removeTaskFlag = true then
        if (removeTaskFlag) {
            // remove ticket - ticketArray
            let ticketId = ticket.childern[1].innerText
            let ticketIndex = ticketArray.findIndex(t =>{
                t.ticketId = ticketId
            })
            ticketArray.splice(ticketId,1) 
...
function handleColor(ticket) {
    ...
    ticketColorBand.addEventListener('click', () => {
        ...
        //updating color int ticketArray if click on tcket color
        //then update color in ticketArray
        ticketArray.forEach(t =>{
            if(t.ticketId == ticketId){
